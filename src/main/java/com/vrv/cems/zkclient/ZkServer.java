...
public class ZkServer {
    ...
    public ZkServer(String dataDir, String logDir, IDefaultNameSpace defaultNameSpace) {
        this(dataDir, logDir, defaultNameSpace, DEFAULT_HOST, DEFAULT_PORT);
    }

    public ZkServer(String dataDir, String logDir, IDefaultNameSpace defaultNameSpace, String ip,int port) {
        this(dataDir, logDir, defaultNameSpace, ip, port, DEFAULT_TICK_TIME);
    }

    public ZkServer(String dataDir, String logDir, IDefaultNameSpace defaultNameSpace, String ip, int port, int tickTime) {
        this(dataDir, logDir, defaultNameSpace, ip ,port, tickTime, DEFAULT_MIN_SESSION_TIMEOUT);
    }

    public ZkServer(String dataDir, String logDir, IDefaultNameSpace defaultNameSpace, String ip, int port, int tickTime, int minSessionTimeout) {
        _dataDir = dataDir;
        _logDir = logDir;
    ...
    public void start() {
        ...
        LOG.info("Starting ZkServer on: [" + _ip + "] port " + _port + "...");
        try {
            //此处是判断zk地址是否是本地服务，如果是就启动
            startZooKeeperServer();
            //想通过zkClient中的watch机制判断zk状态
            _zkClient = new ZkClient(_ip +":" + _port, 10000);
    ...
    private void startZooKeeperServer() {
        ...
        final String servers = _ip+":" + _port;
