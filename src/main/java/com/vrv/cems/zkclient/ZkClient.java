...
import com.vrv.cems.zkclient.exception.ZkAuthFailedException;
import com.vrv.cems.zkclient.exception.ZkBadVersionException;
import com.vrv.cems.zkclient.exception.ZkException;
import com.vrv.cems.zkclient.exception.ZkInterruptedException;
import com.vrv.cems.zkclient.exception.ZkNoNodeException;
import com.vrv.cems.zkclient.exception.ZkNodeExistsException;
import com.vrv.cems.zkclient.exception.ZkTimeoutException;
import com.vrv.cems.zkclient.serialize.SerializableSerializer;
import com.vrv.cems.zkclient.serialize.ZkSerializer;
import com.vrv.cems.zkclient.util.ZkPathUtil;
...
public class ZkClient implements Watcher {
     ...
     *             if operation was interrupted, or a required reconnection got interruptedoccurred
     * @throws IllegalArgumentException
     *             if called from anything except the ZooKeeper event thread
     * @throws ZkException
     *             if any ZooKeeper exception
    ...
    public void process(WatchedEvent event) {
        ...
        try {
            ...
            if (stateChanged) {
                //如果状态发生变化
                LOG.debug("event '{" + event.getType() + " | " + event.getPath() + "}' stateChanged");
                processStateChanged(event);
            }
            if (dataChanged) {
                //数据发生变化
                LOG.debug("event '{" + event.getType() + " | " + event.getPath() + "}' dataChanged");
    ...
    private void processStateChanged(WatchedEvent event) {
        ...
        fireStateChangedEvent(event.getState(),this);
        //如果过期了需要重连接
        if (event.getState() == KeeperState.Expired
            ...
            fireNewSessionEvents(this);
        }
    }

    private void fireNewSessionEvents(final ZkClient zkClient) {
        for (final IZkStateListener stateListener : _stateListener) {
            _eventThread.send(new ZkEventThread.ZkEvent("New session event sent to " + stateListener) {

                @Override
                public void run() throws Exception {
                    stateListener.handleNewSession(zkClient);
                }
            });
        }
    }

    private void fireStateChangedEvent(final KeeperState state,final ZkClient zkClient) {
        for (final IZkStateListener stateListener : _stateListener) {
            _eventThread.send(new ZkEventThread.ZkEvent("State changed to " + state + " sent to " + stateListener) {

                @Override
                public void run() throws Exception {
                    stateListener.handleStateChanged(state,zkClient);
                }
            });
        }
    }
     ...
     * @throws ZkInterruptedException
     */
    public void closeConnection() throws ZkInterruptedException {
        LOG.debug("Closing ZkClient connection...");
        getEventLock().lock();
        try {
            if(_connection!=null){
                _connection.close();
        ...
        LOG.debug("Closing ZkClient connection...done");
    }
    ...
    private void reconnect() {
        ...
        try {
            _connection.close();
            final ZkClient zkClient = this;
            retryUntilConnected(new Callable<ZkClient>() {

                @Override
                public ZkClient call() throws Exception {
                    _connection.connect(zkClient);
                    return zkClient;
                }
            });
