...
public class ServerZkClientTest extends AbstractBaseZkClientTest {
    ...
    public void setUp() throws Exception {
        super.setUp();
        _zkServer = TestUtil.startZkServer(_temporaryFolder, "192.168.133.62",2181);
        //_client = _zkServer.getZkClient();
        _client = new ZkClient("192.168.133.62:2181", CONNECTION_TIMEOUT);
    }
    ...
    public void testRetryUntilConnected() throws Exception {
        ...
        Gateway gateway = new Gateway(4712, "192.168.133.62",2181);
        gateway.start();
        //通过看门程序访问
        final ZkConnection zkConnection = new ZkConnection("192.168.133.62:4712");
    ...
    public void testReadWithTimeout() throws Exception {
        final ZkClient zkClient = new ZkClient("192.168.133.62:4711", 5000, CONNECTION_TIMEOUT, new SerializableSerializer(), 5000);
        // shutdown the server
    ...
    public void testRetryWithTimeout() throws Exception {
        final ZkClient zkClient = new ZkClient("192.168.133.62:2181", CONNECTION_TIMEOUT, CONNECTION_TIMEOUT, new SerializableSerializer(), 5000);
        // shutdown the server
        LOG.info("Shutting down zookeeper server " + _zkServer);
        _zkServer.shutdown();
    ...
    public void testRetryUntilConnected_SessionExpiredException() {
        ...
        final ZkClient zkClient = new ZkClient("192.168.133.62:4712", 200, CONNECTION_TIMEOUT);
    ...
    public void testChildListenerAfterSessionExpiredException() throws Exception {
        ...
        //Gateway gateway = new Gateway(4712, "192.168.133.62",4711);
        //gateway.start();

        final ZkClient disconnectedZkClient = new ZkClient("192.168.133.63:2181", sessionTimeout, CONNECTION_TIMEOUT);
    ...
    public void testZkClientConnectedToGatewayClosesQuickly() throws Exception {
        ...
        final Gateway gateway = new Gateway(4712, "192.168.133.62",4711);
        gateway.start();
